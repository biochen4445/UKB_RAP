...
exome_file_dir="${project}:/Bulk/Exome sequences/Population level exome OQFE variants, PLINK format - interim 450k release/"
data_field="23149"
data_file_dir="${project}:/Data/"


for chr in {1..2}; do
  run_regenie_cmd="regenie --step 2 --bed ukb${data_field}_c${chr}_b0_v2 --out assoc.c${chr}\
  ...
  dx run swiss-army-knife -iin="${exome_file_dir}/ukb${data_field}_c${chr}_b0_v2.bed" \ #bed file for chromosome
   -iin="${exome_file_dir}/ukb${data_field}_c${chr}_b0_v2.bim" \ #bim file for chromosome
   -iin="${exome_file_dir}/ukb2${data_field}_c${chr}_b0_v2.fam"\ #fam file for chromosome
